...
object Int extends BigInteger.Impl with OrderedRing[BigInteger] {
  def number: Parser[BigInteger] = """\d+""".r ^^ { this(_) }
  ...
  def factor(x: BigInteger): Map[BigInteger, Int] = {
    assert(x > 0)
    factor(x, SortedMap.empty[BigInteger, Int](using this), primes)
  }
