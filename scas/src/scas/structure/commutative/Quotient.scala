...
trait Quotient[T : UniqueFactorizationDomain.Impl] extends Quotient.Impl[T] with Field[Element[T]] {
...
object Quotient {
  ...
  trait Impl[T](using ring: UniqueFactorizationDomain.Impl[T]) extends Field.Impl[Element[T]] {
    ...
    extension (ring: UniqueFactorizationDomain.Impl[T]) def apply() = this
  }
}
