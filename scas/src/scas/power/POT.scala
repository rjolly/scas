...
open class POT[N : {Numeric as numeric, ClassTag}](factory: ArrayPowerProduct[N], name: String, dimension: Int) extends ArrayPowerProduct[N] {
  ...
  override def empty = new Array[N](length + dimension)
  ...
  override def gcd(x: Array[N], y: Array[N]): Array[N] = {
    ...
    for i <- 0 until dimension do {
      r(length + i) = numeric.min(x(length + i), y(length + i))
    }
    r
  }
  override def lcm(x: Array[N], y: Array[N]): Array[N] = {
    val r = super.lcm(x, y)
    for i <- 0 until dimension do {
      r(length + i) = numeric.max(x(length + i), y(length + i))
    }
    r
  }
  extension (x: Array[N]) {
    override def multiply(y: Array[N]) = {
      ...
      var i = 0
      while i < dimension do {
        r(length + i) = x(length + i) + y(length + i)
        i += 1
      }
      r
    }
