...
object TreePolynomial {
  ...
  object Factory {
    given coef2poly[D, C, M](using Conversion[D, C], Factory[C, M]): Conversion[D, Element[C, M]] = (x: D) => (x: C): Element[C, M]
    given coef2poly[C, M](using Factory[C, M]): Conversion[C, Element[C, M]] = summon[Factory[C, M]].fromRing(_)
  }
}
