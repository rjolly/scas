...
abstract class TreePolynomial[C, M](using ring: Ring[C], pp: PowerProduct[M]) extends Polynomial[Element[C, M], C, M] {
...
object TreePolynomial {
  ...
  trait Ops[C, M] extends Polynomial.Ops[Element[C, M], C, M] { this: TreePolynomial[C, M] =>
  }
}
