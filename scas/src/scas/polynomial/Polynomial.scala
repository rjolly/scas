...
trait Polynomial[T : ClassTag, C : Ring.Impl, M : PowerProduct.Impl] extends Polynomial.Impl[T, C, M] with Ring[T] {
  extension (x: T) {
...
object Polynomial {
  trait Impl[T : ClassTag, C, M](using ring: Ring.Impl[C], pp: PowerProduct.Impl[M]) extends Ring.Impl[T] {
    val zero = this()
    ...
    extension (ring: Ring.Impl[C]) def apply(s: T*): Impl[T, C, M] = {
