...
trait SolvablePolynomial[T : ClassTag, C : Ring.Impl, M : PowerProduct.Impl] extends SolvablePolynomial.Impl[T, C, M] with Polynomial[T, C, M]

object SolvablePolynomial {
  trait Impl[T : ClassTag, C : Ring.Impl, M](using pp: PowerProduct.Impl[M]) extends Polynomial.Impl[T, C, M] {
    ...
    def lookup(e: M, f: M): Relation = {
      ...
      val list = table.asScala.getOrElse(key, Nil)
      list match {
        case Nil => Relation(pp.one, pp.one, fromPowerProduct(e.multiply(f)))
        case _ => {
    ...
    extension (ring: Polynomial.Impl[T, C, M]) def apply(s: T*): Impl[T, C, M] = {
    ...
    extension (e: M) def ppMultiply(f: M): T = {
      ...
      if (ep.length == 0 || fp.length == 0) fromPowerProduct(e.multiply(f)) else {
        val el = ep(ep.length-1)
        val fl = fp(0)
        if (el <= fl) fromPowerProduct(e.multiply(f)) else {
