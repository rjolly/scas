...
trait MultivariatePolynomial[T[C, M], C : UniqueFactorizationDomain.Impl, M : PowerProduct.Impl](using ClassTag[T[C, M]]) extends MultivariatePolynomial.Impl[T, C, M] with PolynomialOverUFD[T[C, M], C, M]

object MultivariatePolynomial {
  trait Impl[T[C, M], C, M](using ClassTag[T[C, M]])(using ring: UniqueFactorizationDomain.Impl[C], pp: PowerProduct.Impl[M]) extends PolynomialOverUFD.Impl[T[C, M], C, M] {
    ...
    extension (x: T[C, M]) def convertTo(s: MultivariatePolynomial[T, T[C, M], M]): T[T[C, M], M] = iterator(x).foldLeft(s.zero) { (l, r) =>
      ...
      l.add(s(t, this(m.divide(t), c)).convert(variables))
    }
