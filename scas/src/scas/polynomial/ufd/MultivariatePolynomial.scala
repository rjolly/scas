...
trait MultivariatePolynomial[T[C, M], C, M](using ClassTag[T[C, M]])(using val ring: UniqueFactorizationDomain[C], val pp: PowerProduct[M]) extends PolynomialOverUFD[T[C, M], C, M] {
  ...
  override def gcd(x: T[C, M], y: T[C, M]) = if (location > 0) {
    ...
    primitivePart(gcd1(p, q)).multiply(ring.gcd(a, b))
  }
  extension (x: T[C, M]) def convertTo(s: MultivariatePolynomial[T, T[C, M], M]): T[T[C, M], M] = iterator(x).foldLeft(s.zero) { (l, r) =>
    ...
    l.add(s(t, this(m / t, c)).convert(variables))
  }
