...
class PolynomialWithSimpleGCD[C, M](using ring: UniqueFactorizationDomain[C], pp: PowerProduct[M]) extends TreePolynomial[C, M] with scas.polynomial.ufd.impl.MultivariatePolynomial[Element, C, M] with scas.polynomial.ufd.impl.PolynomialWithSimpleGCD[Element[C, M], C, M] with PolynomialOverUFD[Element[C, M], C, M] {
  given instance: PolynomialWithSimpleGCD[C, M] = this
  def split = new PolynomialWithSimpleGCD(using new PolynomialWithSimpleGCD(using ring, pp.take(location)), pp.drop(location))
}
