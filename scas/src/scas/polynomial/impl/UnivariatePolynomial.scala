...
trait UnivariatePolynomial[T, C, M](using ring: Field[C], pp: PowerProduct[M]) extends PolynomialOverField[T, C, M] with EuclidianDomain[T] {
  ...
  extension (x: T) def coefficient(n: BigInteger) = x.coefficient(pp.generator(0) \ n)
}
